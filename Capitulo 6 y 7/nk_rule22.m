...
dynare nk1b.mod noclearall nolog;
M_.Sigma_e = diag([0, 1]); % solo demanda
...
for i = 1:n_simulations
    ...
    losses(i) = 0.5 * (coeff_y * var(y_gap_series) + coeff_pi * var(pi_series));
end
...
fprintf('\nPérdida promedio (Expectativas – demanda): %.4f\n', mean_loss);
fprintf('Varianza promedio brecha de producto: %.4f\n', mean_var_y_gap);
fprintf('Varianza promedio inflación: %.4f\n', mean_var_pi);

save('losses_rule22.mat', 'losses', 'mean_loss', 'mean_var_y_gap', 'mean_var_pi');

figure;
histogram(losses, 50, 'FaceColor', [0.2 0.6 0.8], 'EdgeColor', 'white');
title('Pérdida – Taylor Expectativas (Shock Demanda)');
grid on;
saveas(gcf, 'hist_rule22.png');
