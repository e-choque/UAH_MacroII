...
M_.Sigma_e = diag([1, 1]); % solo tecnología
...
fprintf('\nPérdida promedio (Expectativas – ambos): %.4f\n', mean_loss);
fprintf('Varianza promedio brecha de producto: %.4f\n', mean_var_y_gap);
fprintf('Varianza promedio inflación: %.4f\n', mean_var_pi);

save('losses_rule23.mat', 'losses', 'mean_loss', 'mean_var_y_gap', 'mean_var_pi');

figure;
histogram(losses, 50, 'FaceColor', [0.2 0.6 0.8], 'EdgeColor', 'white');
title('Pérdida – Taylor Expectativas (Shocks tecnología + demanda)');
grid on;
saveas(gcf, 'hist_rule23.png');
